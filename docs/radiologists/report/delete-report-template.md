# Como Excluir um Modelo de Laudo

Aprenda como remover modelos de laudos que não são mais necessários na plataforma UniRadio.

## Passo a Passo

### 1. Acesse a plataforma UniRadio

Faça login em sua conta e navegue até [app.uniradio.vet.br/exams](https://app.uniradio.vet.br/exams)

### 2. Acesse "Minha Conta"

Clique em **"Minha Conta"** no menu para acessar as configurações da sua conta.

![Minha Conta](/images/radiologists/report/delete-report-template/01-my-account.jpeg)

### 3. Acesse "Modelos de Laudos"

Clique em **"Modelos de Laudos"** para visualizar a lista de modelos cadastrados.

![Modelos de Laudos](/images/radiologists/report/delete-report-template/02-templates-reports.jpeg)

### 4. Clique no ícone de lixeira

Localize o modelo que deseja remover e clique no ícone de lixeira correspondente.

![Deletar modelo](/images/radiologists/report/delete-report-template/03-deletar.jpeg)

### 5. Clique em "Continuar" para confirmar

Confirme a exclusão clicando em **"Continuar"**. O modelo será removido permanentemente.

![Confirmar exclusão](/images/radiologists/report/delete-report-template/04-confirm.jpeg)

## Precisa de Ajuda?

Confira nossas [Perguntas Frequentes](/perguntas-frequentes) ou entre em contato com o suporte se encontrar problemas ao excluir modelos de laudos.
