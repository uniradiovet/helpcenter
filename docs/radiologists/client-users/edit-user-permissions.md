# Como Editar as Permissões de um Usuário da Conta de um Cliente

Aprenda como configurar as permissões de acesso de um usuário vinculado à conta de um cliente na plataforma UniRadio.

## Passo a Passo

### 1. Acesse a plataforma UniRadio

Faça login em sua conta e navegue até [app.uniradio.vet.br/exams](https://app.uniradio.vet.br/exams)

### 2. Acesse "Clientes"

No menu lateral, clique em **"Clientes"** para visualizar a lista de clientes cadastrados.

![Clientes](/images/radiologists/client-users/edit-user-permissions/01-clients.jpeg)

### 3. Selecione a clínica

Clique sobre a clínica para abrir a página de gerenciamento.

![Selecionar clínica](/images/radiologists/client-users/edit-user-permissions/02-select-clinic.jpeg)

### 4. Acesse "Usuários da Conta"

Dentro do perfil do cliente, clique em **"Usuários da Conta"** para visualizar os usuários vinculados a ele.

![Usuários da Conta](/images/radiologists/client-users/edit-user-permissions/03-users-account.jpeg)

### 5. Selecione o usuário

Clique sobre o nome do usuário cujas permissões deseja editar.

![Selecionar usuário](/images/radiologists/client-users/edit-user-permissions/04-select-user.jpeg)

### 6. Clique em "Permissões"

Na página do usuário, clique na aba **"Permissões"** para acessar as configurações de acesso.

![Permissões](/images/radiologists/client-users/edit-user-permissions/05-permissions.jpeg)

### 7. Selecione a função do usuário

Clique no dropdown para escolher a função do usuário: **Administrativo** (acesso completo) ou **Membro** (acesso configurável).

![Dropdown de função](/images/radiologists/client-users/edit-user-permissions/06-dropdown.jpeg)

### 8. Configure as permissões (se for Membro)

Caso a função selecionada seja **Membro**, marque as permissões específicas que o usuário deve ter.

![Selecionar permissões](/images/radiologists/client-users/edit-user-permissions/07-select-permissions.jpeg)

### 9. Clique em "Salvar"

Após configurar as permissões, clique em **"Salvar"** para confirmar as alterações.

![Salvar](/images/radiologists/client-users/edit-user-permissions/08-save.jpeg)

## Precisa de Ajuda?

Confira nossas [Perguntas Frequentes](/faq) ou entre em contato com o suporte se encontrar problemas ao editar permissões de usuários.
