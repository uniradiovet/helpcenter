# Como Obter o Link de Redefinição de Senha de um Usuário da Conta

Aprenda como gerar e copiar o link de redefinição de senha de um usuário da sua conta na plataforma UniRadio.

## Passo a Passo

### 1. Acesse a plataforma UniRadio

Faça login em sua conta e navegue até [app.uniradio.vet.br/exams](https://app.uniradio.vet.br/exams)

### 2. Acesse o menu de configurações

No menu lateral, clique no ícone de configurações para abrir as opções da conta.

![Menu de configurações](/images/radiologists/users/account-user-password-reset-link/01-menu.jpeg)

### 3. Clique em "Usuários"

No menu de configurações, clique em **"Usuários"** para visualizar os membros da sua conta.

![Usuários](/images/radiologists/users/account-user-password-reset-link/02-users.jpeg)

### 4. Selecione o usuário

Clique sobre o nome do usuário para o qual deseja gerar o link de redefinição de senha.

![Selecionar usuário](/images/radiologists/users/account-user-password-reset-link/03-select-user.jpeg)

### 5. Clique em "Ações"

Na página do usuário, clique no botão **"Ações"** para exibir as opções disponíveis.

![Ações](/images/radiologists/users/account-user-password-reset-link/04-acoes.jpeg)

### 6. Clique em "Link de Redefinição de Senha"

No menu de ações, selecione **"Link de Redefinição de Senha"** para gerar o link de redefinição.

![Link de Redefinição de Senha](/images/radiologists/users/account-user-password-reset-link/05-link-reset-password.jpeg)

### 7. Clique em "Copiar Link"

Clique em **"Copiar Link"** para copiar o link e enviá-lo ao usuário.

![Copiar Link](/images/radiologists/users/account-user-password-reset-link/06-copiar-link.jpeg)

## Precisa de Ajuda?

Confira nossas [Perguntas Frequentes](/perguntas-frequentes) ou entre em contato com o suporte se encontrar problemas ao gerar links de redefinição de senha.
